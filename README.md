...
- **Token accounting** — sums current context tokens (`input + output + reasoning`) from the latest completed assistant message, falling back to session aggregates
...
> **Version 0.1.2 warning:** npm `0.1.2` incorrectly includes `cache.read` and `cache.write` in context usage. Upgrade to the latest release for the fix. Until then, use the [local source configuration](#from-source) below or disable the npm-installed gauge.
...
- **Used tokens**: the most recent assistant message with output tokens provides `input + output + reasoning`. Cache read/write counters are billing/usage counters and are intentionally excluded from current context-window occupancy. Before any responses exist, session-level token/cost aggregates are used.
